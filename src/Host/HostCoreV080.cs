...
namespace KineticNapier.ADOFAIWorkbench.Host
{
    ...
    internal sealed class TcpHostForm : Form
    {
        ...
        private static readonly string StateDirectory = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "ADOFAIWorkbench");
        ...
        private void ConfigureDockPanel()
        {
            ...
            tabFont = new Font("Segoe UI", 12f, FontStyle.Regular, GraphicsUnit.Point);
        ...
        internal void EnsureVisibleAndForeground()
        {
            ...
            if (!visible)
            {
                ...
                Bounds = new Rectangle(area.Left + Math.Max(0, (area.Width - width) / 2), area.Top + Math.Max(0, (area.Height - height) / 2), width, height);
        ...
        private void RebuildPanesUi()
        {
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                ToolStripButton shortcut = new ToolStripButton(state.Title ?? id) { DisplayStyle = ToolStripItemDisplayStyle.Text, AutoSize = true };
        ...
        private void SaveWindowState()
        {
            try
            {
                ...
                File.WriteAllText(WindowPath, r.X + "|" + r.Y + "|" + r.Width + "|" + r.Height + "|" + (WindowState == FormWindowState.Maximized ? "max" : "normal"));
            }
            catch { }
        }
        ...
        {
            if (disposing && tabFont != null) { tabFont.Dispose(); tabFont = null; }
    ...
    internal sealed class TcpPaneContent : DockContent
    {
        ...
        {
            int count = 0;
            for (int i = 0; i < specs.Count; i++) if (IsRenderable(specs[i].Kind)) count++;
            return count;
        }
        ...
        private void UpdateExisting(List<UiSpec> specs, string payload)
        {
            ...
            try
            {
                ...
                {
                    UiSpec spec = specs[i];
                    if (!IsRenderable(spec.Kind)) continue;
                    ApplySpec(renderedControls[controlIndex++], spec);
                }
        ...
        private Control CreateControl(UiSpec spec)
        {
            if (spec.Kind == "T")
            {
                Label label = new Label { AutoSize = true, MaximumSize = new Size(1200, 0), ForeColor = root.ForeColor, Margin = new Padding(2, 2, 2, 5) };
                ApplySpec(label, spec);
                return label;
            }
            if (spec.Kind == "B")
            {
                Button button = new Button { AutoSize = true, MinimumSize = new Size(78, 34), Margin = new Padding(2), Padding = new Padding(8, 1, 8, 1), FlatStyle = FlatStyle.Flat, ForeColor = Color.White };
            ...
            if (spec.Kind == "I")
            {
                TextBox input = new TextBox { Width = 180, Margin = new Padding(2, 4, 6, 4), BackColor = Color.FromArgb(31, 34, 41), ForeColor = Color.White, BorderStyle = BorderStyle.FixedSingle };
                input.KeyDown += OnInputKeyDown;
                input.Leave += OnInputLeave;
                ApplySpec(input, spec);
                return input;
            }
            if (spec.Kind == "C")
            {
                CheckBox box = new CheckBox { AutoSize = true, Margin = new Padding(4, 7, 8, 4), ForeColor = Color.White, BackColor = PaneBack };
        ...
        private void ApplySpec(Control control, UiSpec spec)
        {
            ...
            {
                button.Text = spec.Text ?? string.Empty;
            ...
            TextBox input = control as TextBox;
            if (input != null && spec.Kind == "I")
            {
                InputBinding binding = input.Tag as InputBinding;
                if (binding == null) input.Tag = binding = new InputBinding();
                binding.Action = spec.Action ?? string.Empty;
                string value = spec.Text ?? string.Empty;
                if (!input.Focused && !string.Equals(input.Text, value, StringComparison.Ordinal)) input.Text = value;
                if (!input.Focused) binding.LastCommitted = value;
                return;
            }

            CheckBox box = control as CheckBox;
            if (box != null && spec.Kind == "C")
            {
                ToggleBinding binding = box.Tag as ToggleBinding;
                if (binding == null) box.Tag = binding = new ToggleBinding();
                binding.Action = spec.Action ?? string.Empty;
                binding.Updating = true;
                box.Text = spec.Text ?? string.Empty;
        ...
        private void CommitInput(TextBox input)
        {
            if (input == null) return;
            InputBinding binding = input.Tag as InputBinding;
            if (binding == null || string.IsNullOrEmpty(binding.Action)) return;
            string value = input.Text ?? string.Empty;
            if (string.Equals(value, binding.LastCommitted ?? string.Empty, StringComparison.Ordinal)) return;
            binding.LastCommitted = value;
            connection.SendAction(paneId, binding.Action, value);
        }
