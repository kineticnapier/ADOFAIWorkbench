...
namespace KineticNapier.ADOFAIWorkbench.Host
{
    ...
    internal sealed class HostConnection : IDisposable
    {
        ...
        internal HostConnection(string pipeName) { this.pipeName = pipeName; }
        ...
        internal void Start(Action<string> receive)
        {
            ...
            readerThread = new Thread(ReadLoop) { IsBackground = true, Name = "ADOFAI Workbench Host IPC" };
            readerThread.Start();
        ...
        private void Send(string line)
        {
            lock (writeGate)
            {
                try { if (writer != null) writer.WriteLine(line); } catch { }
            }
        }
    ...
    internal sealed class HostForm : Form
    {
        ...
        private void HandleMessage(string line)
        {
            ...
            switch (parts[0])
            {
                case "SHOW": Show(); Activate(); break;
                case "HIDE": Hide(); break;
                case "OPEN": if (parts.Length >= 2) OpenPane(HostConnection.Decode(parts[1])); break;
                case "SYNC_BEGIN": syncing = true; syncGeneration++; break;
                case "PANE":
                    if (parts.Length >= 4)
                    {
                        string id = HostConnection.Decode(parts[1]);
                        PaneState state;
                        if (!states.TryGetValue(id, out state)) states[id] = state = new PaneState { Id = id };
                ...
                case "VIEW":
                    if (parts.Length >= 3)
                    {
                        string id = HostConnection.Decode(parts[1]);
                        PaneState state;
                        if (!states.TryGetValue(id, out state)) states[id] = state = new PaneState { Id = id, Title = id, CanClose = true };
        ...
        private void RebuildPanesMenu()
        {
            ...
            list.Sort(delegate(PaneState a, PaneState b) { return string.Compare(a.Title, b.Title, StringComparison.OrdinalIgnoreCase); });
        ...
        private IDockContent DeserializeContent(string persistString)
        {
            ...
            PaneContent content;
            if (!contents.TryGetValue(id, out content)) contents[id] = content = new PaneContent(this, connection, id);
            return content;
        }
        ...
        private void Render(string payload)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < lines.Length; i++)
                {
                    ...
                    if (p[0] == "R+")
                    {
                        FlowLayoutPanel row = new FlowLayoutPanel { AutoSize = true, FlowDirection = FlowDirection.LeftToRight, WrapContents = true, Margin = new Padding(0, 2, 0, 4), BackColor = root.BackColor };
                        root.Controls.Add(row);
                        parent = row;
                    }
                    else if (p[0] == "R-") parent = root;
                    else if (p[0] == "T" && p.Length >= 4)
                    {
                        float size;
                        if (!float.TryParse(p[2], NumberStyles.Float, CultureInfo.InvariantCulture, out size)) size = 10f;
                        Label label = new Label { Text = HostConnection.Decode(p[1]), AutoSize = true, MaximumSize = new Size(900, 0), ForeColor = root.ForeColor, Font = new Font(SystemFonts.MessageBoxFont.FontFamily, size, p[3] == "1" ? FontStyle.Bold : FontStyle.Regular), Margin = new Padding(2, 2, 2, 4) };
                        AddTo(parent, label);
                    }
                    else if (p[0] == "B" && p.Length >= 5)
                    {
                        ...
                        Button button = new Button { Text = HostConnection.Decode(p[1]), AutoSize = true, MinimumSize = new Size(70, 30), Margin = new Padding(2), Padding = new Padding(6, 0, 6, 0), FlatStyle = FlatStyle.Flat, BackColor = p[4] == "1" ? Color.FromArgb(70, 86, 118) : Color.FromArgb(50, 54, 64), ForeColor = Color.White };
                    ...
                    else if (p[0] == "S" && p.Length >= 2)
                    {
                        ...
                        AddTo(parent, new Panel { Width = 1, Height = Math.Max(0, h), Margin = new Padding(0) });
                    }
                }
            }
